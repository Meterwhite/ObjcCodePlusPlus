...
#define LET_INIT_SELF_PATH(...) LET_INIT_KEYSp(self,__VA_ARGS__)
...
/// 且的连续
#define THE_TRUES(...)       (CORE_CONNECT_FOREACH_CXT(__VCOPYING_FOREACH_CAL,&&,,__VA_ARGS__))
/// 其一真
#define THE_SOMETRUE(...)    (CORE_CONNECT_FOREACH_CXT(__VCOPYING_FOREACH_CAL,||,,__VA_ARGS__))
/// 其一假
#define THE_SOMEFALSE(...)   (CORE_CONNECT_FOREACH_CXT(__VCOPYING_FOREACH_CAL,||,!,__VA_ARGS__))
/// 非的连续
#define THE_FALSES(...)      (CORE_CONNECT_FOREACH_CXT(__VCOPYING_FOREACH_CAL,&&,!,__VA_ARGS__))
/// 结对地组合 THE_DUP_PAIR(SRC, > 0 &&, < 100)
#define THE_DUP_PAIR(SRC, ...) CORE_CONNECT_FOREACH_CXT(__VCOPYING_FOREACH_ORD,,SRC,__VA_ARGS__)
/// 装箱：数字，结构体，c字符串
